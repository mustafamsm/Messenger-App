        ...
        <div class="row gx-5">
            <div class="col-auto">
                <div class="avatar avatar-online">
                  <img src="${chat.participants[0].avatar_url}" alt="...">
...
$("#chat-list").on("click", "[data-messages]", function (e) {
    ...
    $.get(`/api/conversations/${id}/messages`, function (res) {
        $("#chat-name").text(res.conversation.participants[0].name);
        $("#chat-avatar").attr(
            "src",
            res.conversation.participants[0].avatar_url
        );
       

        for (i in res.messages.data) {
            let className = "";
            if (res.messages.data[i].user_id == userId) {
                className = "message-out";
            }
            addMessage(res.messages.data[i], className);
        }
    });
});
